...
<div
  class="vf-w-full vf-flex vf-flex-row vf-mt-6 vf-px-4 vf-py-3 vf-rounded-md vf-border-l-[6px] vf-border-solid vf-border {$alertColor} vf-bg-white vf-text-xs {if $textcenter} text-center {/if}{if $additionalClasses} {$additionalClasses}{/if}{if $hide} vf-hidden {/if}"
  {if $idname} id="{$idname}" {/if}>
  <div class="vf-text-sm vf-text-center vf-mr-4"><i class="far fa-bell"></i></div>
  <div class="vf-text-sm alert-msg">
    {$msg}
  </div>
</div>