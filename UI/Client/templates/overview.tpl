...
<div class="vf-flex-1 vf-flex vf-flex-col" id="vnx-fw-overview">
    {if $status == "Active" || $status == "Đang sử dụng"}

        <div class="vf-flex vf-flex-row " id="alertMsg">
            <div v-if="alertMsg != ''" :class="[[alertType]]" class="vf-w-full vf-flex vf-flex-row vf-my-2 vf-px-4 vf-py-3 vf-rounded-md vf-border-l-[6px] vf-border-solid vf-border vf-bg-white vf-text-xs">
            ...
            <div class="vf-flex vf-flex-col md:vf-flex-row">
                <div class="vf-flex-1 vf-flex vf-flex-col vf-border-b vf-border-solid vf-pb-4 vf-mb-4 md:vf-pb-0 md:vf-mb-0 md:vf-border-r md:vf-border-b-0">
                    <div class="vf-mr-5 vf-ml-7">
                        <img class="vf-w-12 vf-h-12" src="{$WEB_ROOT}/modules/servers/vietnix_firewall2/UI/Client/assets/img/firewall.svg" />
                    </div>
                ...
                <div class="vf-flex-1 vf-flex vf-flex-col vf-justify-end vf-px-6 md:vf-justify-center">
                    ...
                    <div class="vf-flex vf-flex-row vf-mb-4">
                        ...
                        <div class="vf-flex-1"></div>
                        <div class="vf-bg-primary vf-rounded-[900px] vf-text-xs vf-text-white vf-font-bold vf-py-1 vf-px-3" id="fw-data-transfer">Đang tải...</div>
                    </div>

                    <div class="vf-flex vf-flex-row">
                        <div class="vf-text-xs vf-text-gray-1">Block IP International</div>
                        <div class="vf-flex-1"></div>
                        <div id="block-ip-status">
                            <div v-if="loading==false" class="vf-flex vf-justify-center">
                                <span class=" vf-text-xs vf-text-gray-1" id="block-ip-status-label">
                                    [[blockIpStatus==true ? "Yes" : "No"]]
                                </span>
                                <!-- toggle -->
                                <div @click="toggleBlockIP(blockIpStatus==true ? 'unblock' : 'block')" class="vf-relative vf-ml-2 vf-cursor-pointer">
                                    <!-- input -->
                                    <input :checked="blockIpStatus==true" type="checkbox" class="vf-sr-only vf-border" value="yes">
                                    <!-- line -->
                            ...
                            <div v-else class="vf-ml-2"><i class="fas fa-spinner fa-spin vf-text-primary"></i></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="vf-flex vf-flex-col vf-bg-white vf-rounded vf-shadow-card vf-py-6 vf-mt-6">
            ...
            <div class="vf-flex-1 vf-mr-5 vf-ml-7 vf-mt-2">
                <div id="fwchart" class="vf-center vf-flex-col">
                    <div class="vf-mt-2"><img class="vf-w-12 vf-h-12" src="{$WEB_ROOT}/modules/servers/vietnix_firewall2/UI/Client/assets/img/loading.gif" /></div>
                    <div class="vf-text-sm vf-text-gray-1 vf-mt-2">Đang tải dữ liệu ...</div>
                </div>
            </div>
        </div>
    {else}
        {include file="./Components/alert.tpl" type="error" msg="Tình trạng dịch vụ này: {$status}"}
    {/if}
    <!-- Payment Info -->
    <div class="vf-flex vf-flex-col vf-bg-white vf-rounded vf-shadow-card vf-py-6 vf-mt-6">
...
<script>
    Helper.initOverviewData({$id},'{$regdate}', '{$dedicatedip}', {$serviceid});
